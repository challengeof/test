...
public class Controller implements ControllerSupport, LocalVariablesSupport {
     ...
     * You can store value associated with keys and later retrieve them.
     * It has one special property: by default, value stored into the flash during the processing of a request
     * will be available during the processing of the immediately following request.
     * Once that second request has been processed, those value are removed automatically from the storage
     *
    ...
    protected static void renderTemplate(String templateName, Map<String,Object> args) {
        ...
        try {
            ...
            RenderTemplate renderTemplate = new RenderTemplate(template, templateBinding.data);
     ...
     * E.g. "controllers.Pages.index" returns "views/Pages/community.html".
     */
    ...
    private static void storeOrRestoreDataStateForContinuations(Boolean isRestoring) {
        ...
        if (isRestoring) {
            ...
            LocalVariablesNamesTracer.setLocalVariablesStateAfterAwait(localVariablesState);
            ...
            // Params
            // We know that the params are partially reprocessed during awake(Before now), but here we restore the correct value as
            ...
            // Params
            // Store the actual params value so we can restore the exact same state when awaking.
